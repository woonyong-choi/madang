...
- 프로젝트는 core에 등록한 로컬 폴더(보통 git 저장소)다. 페이지 기록은 그 폴더의 `.madang/`에
  있고, 전역 설정과 root.md는 앱 홈(`~/.madang`)에 있다. 둘 다 core가 쓴다.
...
- 메모리 패널(3열 위 "메모리" 또는 M): root / project / state를 고쳐 저장한다. core 검사기가 거부하면
...
앱 홈 상태(`GET/POST /home`)와 라우팅 표(`GET/PUT /config/routes`)도 생성 클라이언트(`SetupApi`)로
부른다.
