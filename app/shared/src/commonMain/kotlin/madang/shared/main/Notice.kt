...
sealed interface Notice {
    ...
    data class Failed(override val page: String, override val title: String, val error: String) :
        Notice
...
fun noticeOf(payload: Any, titleOf: (String) -> String): Notice? = when (payload) {
    is RunFinishedEvent ->
        Notice.Finished(payload.page, titleOf(payload.page), payload.data.resultStatus)

    is RunFailedEvent -> Notice.Failed(payload.page, titleOf(payload.page), payload.data.error)
    ...
    is FlowWaitingEvent -> payload.data.decision?.let {
        Notice.Asked(payload.page, titleOf(payload.page), it.question.prompt, it.id)
    }
