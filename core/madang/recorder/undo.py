...
부작용 종류:
    ...
    repo: 프로젝트 작업 트리 파일. ``commit``을 채우는 git 모듈이 되감는다.
...
def undo(page_dir: Path, n: int) -> UndoResult:
    """실행 ``n``이 바꾼 페이지 파일을 실행 전 내용으로 되감는다.

    하나라도 기록 뒤에 다시 바뀐 파일이 있으면 아무것도 바꾸지 않는다.
    ...
    Raises:
        ...
        UndoConflictError: 실행 뒤에 다시 바뀐 파일이 있다.
    """
    log = read_log(page_dir, n)
    if log is None:
    ...
    return UndoResult(
        ...
        skipped=[e.path or e.kind for e in log.effects if e.kind != FILE],
    )
