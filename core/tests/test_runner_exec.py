...
def work(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    for key in ("FAKE_STREAM", "FAKE_DUMP", "FAKE_SLEEP", "FAKE_CHILD", "FAKE_STDERR", "FAKE_EXIT"):
...
def test_render_args() -> None:
    ...
    assert render_args(["-m", "{model}", "-c", "model_reasoning_effort={effort}"], values) == [
        "-m", "m", "-c", "model_reasoning_effort=high",
    ]
...
def test_default_commands_come_from_runners_yaml(tmp_path: Path) -> None:
    ...
    cmd = claude.command(cwd=tmp_path, prompt="-hi", model="claude-haiku-4-5", effort="low")
    assert cmd == [
        "claude", "-p", "--output-format", "stream-json", "--verbose",
        "--model", "claude-haiku-4-5", "--effort", "low", "--", "-hi",
    ]
    cmd = codex.command(cwd=tmp_path, prompt="hi", model="gpt-6-luna", effort="low")
    assert cmd == [
        "codex", "exec", "--json", "-m", "gpt-6-luna", "-c", "model_reasoning_effort=low", "--", "hi",
    ]
...
def test_prompt_placeholder_and_home_dir(tmp_path: Path) -> None:
    spec = RunnerSpec(bin="codex", args=["exec", "--add-dir", "{home}", "{prompt}", "-m", "{model}"])
    runner = CodexRunner(spec, home=tmp_path / "home")
    cmd = runner.command(cwd=tmp_path, prompt="go", model="m", effort="low")
    assert cmd == ["codex", "exec", "--add-dir", str(tmp_path / "home"), "go", "-m", "m"]
...
def test_exec_replays_stream(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                             monkeypatch: pytest.MonkeyPatch) -> None:
    dump = tmp_path / "dump.json"
    monkeypatch.setenv("FAKE_STREAM", str(STREAMS / "claude-tools.jsonl"))
    monkeypatch.setenv("FAKE_DUMP", str(dump))
    home = tmp_path / "home"
    runner = ClaudeRunner(fake_spec, home=home, core_url="http://127.0.0.1:7470")
    log = tmp_path / "runs" / "1.events.jsonl"
    events, on_event = collect()

    result = runner.exec(cwd=work, prompt="do it", model="claude-haiku-4-5", effort="low",
                         on_event=on_event, page="2026-09-24-demo", events_log=log)
    ...
    assert log.read_text(encoding="utf-8") == (STREAMS / "claude-tools.jsonl").read_text(encoding="utf-8")

    called = json.loads(dump.read_text(encoding="utf-8"))
    assert called["argv"] == [
        "--model", "claude-haiku-4-5", "--effort", "low", "--add-dir", str(home), "--", "do it",
    ]
...
def test_exec_without_page_drops_inherited_page(work: Path, tmp_path: Path,
...
def test_exec_error_result(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                           monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = ClaudeRunner(fake_spec, home=tmp_path).exec(
        cwd=work, prompt="p", model="claude-nope-9", effort="low", on_event=on_event)
...
def test_exec_nonzero_exit_uses_stderr(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                                       monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_exec_stream_without_result_is_error(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                                             monkeypatch: pytest.MonkeyPatch) -> None:
    truncated = tmp_path / "cut.jsonl"
    truncated.write_text((STREAMS / "codex-ok.jsonl").read_text().splitlines()[0] + "\nnot json\n")
...
def test_exec_missing_binary(work: Path, tmp_path: Path) -> None:
    runner = ClaudeRunner(RunnerSpec(bin=str(tmp_path / "no-such-cli")), home=tmp_path)
    events, on_event = collect()
    result = runner.exec(cwd=work, prompt="p", model="m", effort="low", on_event=on_event)
    assert result.status == "error"
...
def test_timeout_stops_process_group(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                                     monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CodexRunner(fake_spec, home=tmp_path).exec(
        cwd=work, prompt="p", model="m", effort="low", on_event=on_event, timeout=0.5)
...
def test_cancel(work: Path, tmp_path: Path, fake_spec: RunnerSpec,
                monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("FAKE_SLEEP", "30")
    runner = CodexRunner(fake_spec, home=tmp_path)
    results = []
    thread = threading.Thread(target=lambda: results.append(runner.exec(
        cwd=work, prompt="p", model="m", effort="low", on_event=lambda e: None)))
    thread.start()
