  ...
  for (const name of NAMES) {
    test(`${name} has every file, no scripts and a CSP`, () => {
      ...
      for (const file of files) {
        expect(fs.existsSync(path.join(TEMPLATES, name, file)),
            `${name}/${file}`).toBe(true);
      }
      ...
      const html =
        fs.readFileSync(path.join(TEMPLATES, name, 'page.html'), 'utf8');
      ...
      const yaml = fs.readFileSync(
          path.join(TEMPLATES, name, 'template.yaml'), 'utf8');
      ...
      const yamlSlots = [...slotBlock.matchAll(/^\s+([A-Za-z_][\w-]*):/gm)]
          .map((m) => m[1]);
      ...
      for (const slot of entry.slots) {
        expect(schema[slot.schema.replace('#/', '')],
            `${name} schema for ${slot.name}`).toBeTruthy();
        if (data[slot.name] !== undefined) {
          const validate = ajv.getSchema(`schema${slot.schema}`);
          const errors = JSON.stringify(validate.errors);
          expect(validate(data[slot.name]),
              `${name} sample ${slot.name}: ${errors}`).toBe(true);
        }
        if (slot.required) {
          expect(data[slot.name], `${name} sample for ${slot.name}`)
              .toBeTruthy();
        }
      }
    });
...
test.describe('sample rendering', () => {
  test('table renders header and every row', async ({page}) => {
    ...
    await expect(page.locator(pathAttr('rows[1][0]')))
        .toHaveAttribute('data-madang-slot', 'data');
  ...
  test('decisions are grouped by state', async ({page}) => {
    ...
    await expect(
        page.locator('section.confirmed ' + pathAttr('decisions[0].choice')))
        .toHaveText('refresh-lock');
    await expect(page.locator(pathAttr('decisions[0].options[2]')))
        .toHaveText('client-retry');
    await expect(
        page.locator('section.proposed ' + pathAttr('decisions[1].topic')))
        .toHaveText('세션 저장소');
  });
  ...
  test('tasks are split into status columns', async ({page}) => {
    ...
    await expect(page.locator('.doing ' + pathAttr('tasks[1].title')))
        .toHaveText('갱신 잠금');
    await expect(page.locator(pathAttr('tasks[1].due')))
        .toHaveText('2026-09-26');
  ...
  test('resume renders base data', async ({page}) => {
    ...
    await expect(page.locator(pathAttr('contact.links[0]') + ' a'))
        .toHaveAttribute('href', 'https://github.com/madang');
  });

  test('theme variables reach the page', async ({page}) => {
    await openTemplate(page, 'resume', {
      theme: {color: {accent: 'rgb(200, 0, 0)'}, font: {size: 18}},
    });
...
test.describe('slot overlay', () => {
  test('overlay wins and each value keeps its slot', async ({page}) => {
    await openTemplate(page, 'resume');
    const overlay = sample('resume').overlay;
    await expect(page.locator(pathAttr('title'))).toHaveText(overlay.title);
    await expect(page.locator(pathAttr('title')))
        .toHaveAttribute('data-madang-slot', 'overlay');
    await expect(page.locator(pathAttr('summary')))
        .toHaveAttribute('data-madang-slot', 'overlay');
    await expect(page.locator(pathAttr('name')))
        .toHaveAttribute('data-madang-slot', 'base');
    await expect(page.locator(pathAttr('work[1].company')))
        .toHaveAttribute('data-madang-slot', 'base');
    await expect(page.locator('h2').first())
        .toHaveAttribute('data-madang-slot', 'template');
  });
  ...
  test('objects merge deeply and arrays are replaced whole', async ({page}) => {
    ...
    await expect(page.locator(pathAttr('contact.phone')))
        .toHaveText('010-1111-2222');
    await expect(page.locator(pathAttr('contact.phone')))
        .toHaveAttribute('data-madang-slot', 'overlay');
    await expect(page.locator(pathAttr('contact.email')))
        .toHaveAttribute('data-madang-slot', 'base');
    await expect(page.locator('.job')).toHaveCount(1);
    await expect(page.locator(pathAttr('work[0].company'))).toHaveText('넥슨');
    await expect(page.locator(pathAttr('work[0].company')))
        .toHaveAttribute('data-madang-slot', 'overlay');
    const source =
      await page.evaluate(() => window.madang.sourceOf('skills[0]'));
    expect(source).toBe('base');
  });

  test('later template slot wins over data key order', async ({page}) => {
...
test.describe('modes and selection', () => {
  test('edit mode click reports the data path', async ({page}) => {
    ...
    const cursor = await page.locator(pathAttr('work[1].company'))
        .evaluate((el) => getComputedStyle(el).cursor);
    expect(cursor).toBe('crosshair');
  });

  test('unbound child click selects nearest path', async ({page}) => {
    await openTemplate(page, 'resume', {mode: 'edit'});
    await page.locator(pathAttr('work[1]') + ' .job-head')
        .click({position: {x: 350, y: 2}});
  ...
  test('ctrl/cmd click adds, escape clears', async ({page}) => {
    await openTemplate(page, 'resume', {mode: 'edit'});
    await page.locator(pathAttr('work[1].company')).click();
    await page.locator(pathAttr('work[0].company'))
  ...
  test('view mode link click is reported as navigate', async ({page}) => {
    ...
    expect(nav.map((e) => e.payload.url))
        .toEqual(['https://github.com/madang']);
    expect(page.url()).toBe(before);
  });

  test('setMode switches mode, select() highlights paths', async ({page}) => {
    await openTemplate(page, 'tasks', {mode: 'view'});
    await page.evaluate(() => window.madang.setMode('edit'));
    await expect(page.locator('html'))
        .toHaveAttribute('data-madang-mode', 'edit');
    const info = await page.evaluate(() => {
      return window.madang.select(['tasks[1].title', 'tasks[2]']);
    });
...
test.describe('data patching', () => {
  test('patchData repaints at once and keeps selection', async ({page}) => {
    await openTemplate(page, 'resume', {mode: 'edit'});
    await page.locator(pathAttr('work[1].company')).click();
    const res = await page.evaluate(() => {
      return window.madang.patchData('work[1].company', '펄어비스 (서울)');
    });
    expect(res).toEqual({slot: 'base', path: 'work[1].company'});
    await expect(page.locator(pathAttr('work[1].company')))
        .toHaveText('펄어비스 (서울)');
    await expect(page.locator(pathAttr('work[1].company')))
        .toHaveClass(/madang-selected/);
    const data = await page.evaluate(() => window.madang.getData());
    expect(data.base.work[1].company).toBe('펄어비스 (서울)');
  });

  test('patchData writes to the supplying slot', async ({page}) => {
    await openTemplate(page, 'resume');
    const res = await page.evaluate(() => {
      return window.madang.patchData('title', '새 직함');
    });
  ...
  test('patchData refuses fixed template text', async ({page}) => {
    ...
    const res = await page.evaluate((p) => {
      return window.madang.patchData(p, '직장');
    }, tplPath);
    expect(res).toBeNull();
    const error = (await events(page, 'error')).pop();
    expect(error.payload.message).toContain('template');
...
test.describe('bridge', () => {
  test('host adapter receives JSON messages', async ({page}) => {
    await page.addInitScript(() => {
      window.__posted = [];
      window.madangBridge = {
        post: (json) => window.__posted.push(JSON.parse(json)),
      };
    });
    await openTemplate(page, 'resume', {mode: 'edit'});
    await page.locator(pathAttr('name')).click();
    const posted = await page.evaluate(() => window.__posted);
    expect(posted.map((m) => m.type))
        .toEqual(expect.arrayContaining(['ready', 'selected']));
    const selected = posted.find((m) => m.type === 'selected');
    expect(selected.payload.paths).toEqual(['name']);
  ...
  test('render errors are reported', async ({page}) => {
    ...
    const ok = await page.evaluate(() => {
      const template = {
        name: 'x',
        slots: ['a'],
        html: '<p data-bind="nope.x"></p>',
      };
      return window.madang.render(template, {a: {}}, {}, 'view');
    });
    expect(ok).toBe(false);
    const errors = await events(page, 'error');
    expect(errors[0].payload.message).toContain('unknown slot');
    const badMode = await page.evaluate(() => {
      const template = {name: 'x', slots: ['a'], html: '<p></p>'};
      return window.madang.render(template, {a: {}}, {}, 'bogus');
    });
    expect(badMode).toBe(false);
  });

  test('renderMarkdown shows plain text until a renderer', async ({page}) => {
    await openTemplate(page, 'table');
